...
public class NextPieceAndScorePanel extends JPanel {
	...
	public NextPieceAndScorePanel(int width, int height){
		...
		score = new JLabel("0");
		score.setFont(new Font(score.getFont().getFamily(), score.getFont().getStyle(), 23));
		add(score);
		
		level = new JLabel("");
		level.setFont(new Font(level.getFont().getFamily(), level.getFont().getStyle(), 23));
		add(level);
	}
	...
	public void setPiece(Piece piece){
		...
		piece.move(pieceNewXValue, pieceNewYValue);
